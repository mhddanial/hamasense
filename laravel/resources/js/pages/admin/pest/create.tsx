                        ...
                        <div>
                            <div className="space-y-6">
                                ...
                                <div className='w-full px-4 py-2 border border-gray-300 rounded-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-teal-500'>
                                    <DataTableDemo allDatas={plants} checkedDatas={data.plants} onChange={(value) => {
                                        const checkedPlants: number[] = data.plants;
                                        if (!data.plants.includes(value)) {
                                            setData('plants', [...checkedPlants, value]);
                                            return;
                                        }else {
                                            setData('plants', [...checkedPlants.filter((id) => value !== id)]);
                                            return;
                                        }
                                    }} name={'plants'}/>
                                    <BadgeDemo checkedItems={data.plants} allItems={plants} onClick={(id: number) => { setPlantList((plants) => plants.filter((disease: Plant) => disease.id !== id))}}/>
                                </div>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">
                                ...
                                <div className="flex gap-4">
                                    ...
                                    <button onClick={create} type='button' disabled={disableCreate}
